...
let svg = `<svg width="${TASKS_WIDTH}" height="${TASKS_HEIGHT}">\n`;

for (let i = 0; i < 100; i++) {
  const y = 10 + i * (TASK_HEIGHT + MARGIN);
  svg += `
  <!-- Task ${i} -->
  <rect id="bg${i}" x="${X_PADDING}" y="${y}" width="${WIDTH}" height="${TASK_HEIGHT}" rx="8" ry="8" fill="#444444" />
  <text id="task${i}" x="${X_PADDING + 10}" y="${
    y + 28
  }" font-size="24" fill="white" text-length="250" />
  <rect id="tap${i}" x="0" y="${y}" width="348" height="${TASK_HEIGHT}" fill="white" fill-opacity="0.01" />\n`;
}
